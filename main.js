...
let texPivot = [0.5, 0.5];
let texAngle = 0.0;


const TEX_PIVOT_STEP = 0.01;
const TEX_ANGLE_STEP = 0.03;
...
function draw() {
    ...
    const el = document.getElementById("cgwInfo");
    if (el) el.textContent =
    `Pivot (u,v): ${texPivot[0].toFixed(2)}, ${texPivot[1].toFixed(2)} | Angle: ${texAngle.toFixed(2)}`;
...
function loadTexture(url) {
    ...
    // тимчасовий піксель, поки картинка вантажиться
    gl.texImage2D(
...
function initGL() {
    ...
    shProgram.iTexAngle = gl.getUniformLocation(prog, "u_texAngle");
...
function init() {
    ...
    window.addEventListener("keydown", (e) => {
        switch (e.key.toLowerCase()) {
            case "a": texPivot[0] -= TEX_PIVOT_STEP; break; // u-
            case "d": texPivot[0] += TEX_PIVOT_STEP; break; // u+
            case "w": texPivot[1] += TEX_PIVOT_STEP; break; // v+
            case "s": texPivot[1] -= TEX_PIVOT_STEP; break; // v-

            case "q": texAngle -= TEX_ANGLE_STEP; break;
            case "e": texAngle += TEX_ANGLE_STEP; break;

            case "c": texPivot = [0.5, 0.5]; break;
            case "r": texAngle = 0.0; break;
